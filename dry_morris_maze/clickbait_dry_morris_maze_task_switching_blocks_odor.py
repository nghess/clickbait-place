...
def get_platform_corner(flip_state, grid_cells_x, grid_cells_y):
    """Return (col_start, row_start) in full-grid cell units for the given flip_state.
    
    Platform is PLATFORM_COLS wide x PLATFORM_ROWS tall in full-grid cell units.
      flip_state 0 -> upper right: cols (grid_cells_x - PLATFORM_COLS) .. (grid_cells_x - 1), row_start = 1
      flip_state 1 -> lower left:  cols 0 .. (PLATFORM_COLS - 1),            row_start = grid_cells_y - 1 - PLATFORM_ROWS
      flip_state 2 -> lower right: cols (grid_cells_x - PLATFORM_COLS) ..,   row_start = grid_cells_y - 1 - PLATFORM_ROWS
      flip_state 3 -> upper left:  cols 0 .. (PLATFORM_COLS - 1),            row_start = 1
    """
    if flip_state == 0:
        ...
        raise ValueError("Unknown flip_state: %r" % (flip_state,))
...
def get_platform_rect(flip_state, grid_cells_x, grid_cells_y, img_dims):
    ...
    See get_platform_corner for state -> corner mapping.
    Returns (top_left_Point, bottom_right_Point).
    """
    cell_w = img_dims[0] // grid_cells_x
    cell_h = img_dims[1] // grid_cells_y
    
    col_start, row_start = get_platform_corner(flip_state, grid_cells_x, grid_cells_y)
...
# ALLOWED_STATES selects which of the four platform locations are used in this session.
# State -> location:
#   0 = upper right
#   1 = lower left
#   2 = lower right
#   3 = upper left
# Examples:
#   ALLOWED_STATES = [0, 1]        # original two-state task (upper-right / lower-left)
#   ALLOWED_STATES = [2, 3]        # the other diagonal (lower-right / upper-left)
#   ALLOWED_STATES = [0, 1, 2, 3]  # all four
ALLOWED_STATES = [2, 3]

INITIAL_BLOCK_TRIALS = 1          # Trials per initial fixed block (one block per allowed state)
...
def generate_flip_schedule():
    """Build a trial-by-trial flip-state schedule across ALLOWED_STATES.
    
    Phase 1: INITIAL_BLOCK_TRIALS trials per allowed state, in the order given by ALLOWED_STATES.
    Phase 2: pseudorandom — PSEUDORANDOM_TRIALS_PER_STATE trials per allowed state, shuffled
             so that no more than MAX_CONSECUTIVE_SAME_STATE consecutive trials share a state.
    
    With len(ALLOWED_STATES) == 2 and identical totals this reproduces the original
    two-state schedule (modulo the initial-block ordering).
    """
    ...
    # Phase 1: one fixed block per allowed state, in the order listed
    for s in ALLOWED_STATES:
        schedule.extend([s] * INITIAL_BLOCK_TRIALS)
    
    # Phase 2: pseudorandom across all allowed states
    pseudo = []
    for s in ALLOWED_STATES:
        pseudo.extend([s] * PSEUDORANDOM_TRIALS_PER_STATE)
    ...
    # (Single-state special case: nothing to fix; skip.)
    if len(ALLOWED_STATES) > 1:
        for _safety in range(1000):
            ...
            # Search for the nearest safe swap partner (different value, and inserting
            ...
            if best_j is not None:
                ...
                # No safe swap available — reshuffle and retry
                random.shuffle(pseudo)
...
flip_state = flip_schedule[0]  # Start at the first state in the generated schedule
