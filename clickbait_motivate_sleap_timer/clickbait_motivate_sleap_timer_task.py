...
trial_timeout_duration = 5.0  # seconds before a trial expires as "incorrect"

# Flip state parameters
FLIP_STATE_CONFIG = {
    'min_trials_before_flip': 1,    # Minimum trials before state flip
    'max_trials_before_flip': 3,   # Maximum trials before state flip
}
...
DISTRIBUTION_CONFIG_STATE_0 = {
    ...
    'log_sigma_y': 0.4,  # 0.2-0.4: tight, 0.5: moderate, 0.8-1.2: fat tails
...
DISTRIBUTION_CONFIG_STATE_1 = {
    ...
    'mean_y': 1,     # Same mean position as state 0
    ...
    'log_normal': 'y',
    'log_sigma_x': 0.5,
    'log_sigma_y': 0.4,   # Same parameters as state 0
...
def process(value):
    ...
    if trial_active and active_target is not None and not reward_state:
        if current_time - trial_start_time >= trial_timeout_duration:
            # Trial expired — mark as incorrect
            incorrect_count += 1
            active_target = None
            trial_active = False
            
            # Skip reward, go directly to ITI
            in_iti = True
    ...
    if in_iti:
        if current_time - iti_start_time >= iti_duration:
            ...
            if trials_since_last_flip >= trials_until_next_flip:
                ...
                # Start trial timer for the new target
    ...
    return (canvas, Point(centroid_x, centroid_y), reward_state, reward_left, reward_right, 
            ...
            trial_count, reward_left_count, reward_right_count, incorrect_count, tuple(target_distribution), flip_state)