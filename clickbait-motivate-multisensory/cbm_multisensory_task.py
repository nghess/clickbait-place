...
def get_current_distribution_config():
    ...
    return DISTRIBUTION_CONFIGS[flip_state]
...
DISTRIBUTION_CONFIG_STATE_2 = {
    # Duplicate of state 0's configuration (top-focused log-normal).
    # Kept as its own dict so it can be tuned independently of state 0.
    'mean_x': None,
    'mean_y': 2,
...
DISTRIBUTION_CONFIG_STATE_3 = {
    # Duplicate of state 1's configuration (bottom-focused flipped log-normal).
    # Kept as its own dict so it can be tuned independently of state 1.
    'mean_x': None,
    'mean_y': 2,
...
DISTRIBUTION_CONFIG_STATE_4 = {
    # Formerly DISTRIBUTION_CONFIG_STATE_2 — renumbered to 4.
...
# Single lookup table mapping flip_state ID -> distribution config.
DISTRIBUTION_CONFIGS = {
    0: DISTRIBUTION_CONFIG_STATE_0,
    1: DISTRIBUTION_CONFIG_STATE_1,
    2: DISTRIBUTION_CONFIG_STATE_2,
    3: DISTRIBUTION_CONFIG_STATE_3,
    4: DISTRIBUTION_CONFIG_STATE_4,
}

# Ordered list of active flip state IDs — drives schedule generation and
# per-state queue initialization.
FLIP_STATES = [0, 1, 2, 3, 4]
...
# -------------------------------------------------------------------
# Pseudorandom flip schedule: TRIALS_PER_STATE trials of each state in
# FLIP_STATES (0, 1, 2, 3, 4), permuted such that no run of identical
# states exceeds MAX_CONSECUTIVE_SAME_STATE. Direct port of the algorithm
# in clickbait_dry_morris_maze_task_switching_blocks.py, generalized
# to an arbitrary list of state IDs.
# -------------------------------------------------------------------
TRIALS_PER_STATE = 50
...
def generate_flip_schedule():
    ...
    TRIALS_PER_STATE trials of each state in FLIP_STATES, shuffled so that
    no run of the same state exceeds MAX_CONSECUTIVE_SAME_STATE.
    """
    schedule = []
    for _s in FLIP_STATES:
        schedule.extend([_s] * TRIALS_PER_STATE)
...
# Keyed by flip_state ID (0, 1, 2, 3, 4). Dicts keyed by ID rather than
# positional lists, so IDs stay explicit. Encountered targets are NOT
# regenerated when flip_state changes — each state's queue persists across
# flips and only regenerates when its own queue is fully exhausted.
...
for _state in FLIP_STATES:
    _cfg = DISTRIBUTION_CONFIGS[_state]
